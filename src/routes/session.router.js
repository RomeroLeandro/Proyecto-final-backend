...
const sendPasswordResetMail = require("../middleware/password.mail");
...
sessionRouter.post("/reset-password", async (req, res) => {
  ...
  try {
    ...
    const resetToken = jwt.sign({ userId: user._id }, env.session.secret, {
      expiresIn: "1h",
    });
...
sessionRouter.post("/reset-password/:resetToken", async (req, res) => {
  ...
  try {
    ...
    const { userId } = jwt.verify(decodedToken, env.session.secret);
